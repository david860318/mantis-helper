...
export function useMantisParser() {
  ...
  const importantData = reactive<Record<string, string[]>>({})
  const urgentData = reactive<Record<string, string[]>>({})
  const pendingReleaseData = reactive<any[]>([])

  function clear(obj: Record<string, any>) {
